					...
					checkout(
						[
							...
							extensions: [
								...
								[
									$class: 'UserIdentity',
									email: "suraj.patil94227@gmail.com",
									name: "suraj patil"
								],
							],
							userRemoteConfigs: [[url: 'https://github.com/daddybapu/devops_ci_cd.git']]						
						]
						)
	...
	stage('Deploying on Docker') {
		...
		steps {
			script {
				if (env.BRANCH_NAME == 'master'){
				sh """
				if [ `curl -o /dev/null -s -w "%{http_code}\n"http://13.233.147.37:83/index.html` = 200 ];
				then
				...
				sh """
				if [ `curl -o /dev/null -s -w "%{http_code}\n"http://13.233.147.37:82/index.html` = 200 ];
				then
				echo "Merge Successful"
				else
				echo "Merge Failed"
	...
	post {
		success {
			...
			if (env.BRANCH_NAME == 'master'){
				...
				sh('git push https://${GIT_USERNAME}:${GIT_PASSWORD}@github.com/daddybapu/devops_ci_cd.git HEAD:master')
					}
				} else if (env.BRANCH_NAME == 'dev'){
